...
<body>
    <h1>New Person</h1>
    <form:form method="POST" action="/licenses/new/create" modelAttribute="license">
        <form:label path="person">Person
        <form:errors path="person"/>
		<form:select path="person">
        	<c:forEach var="person" items="${ persons }">
        		<c:if test="${ person.license == null }">
        		<option value="${ person.id }"> <c:out value="${person.firstName }"/> </option>
        		</c:if>
        	</c:forEach>
        ...
        <form:hidden value="${ number }" path="number"/>
